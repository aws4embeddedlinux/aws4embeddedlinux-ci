    ...
    test('Creates EFS filesystem with security group', () => {
        ...
        template.hasResourceProperties('AWS::EC2::SecurityGroup', {
            SecurityGroupIngress: Match.arrayWith([Match.objectLike(
                {
                    CidrIp: {
                        "Fn::ImportValue": Match.stringLikeRegexp(`${common.stackName}:ExportsOutputFnGetAtt${props.vpc.node.id.replace('-', '')}*`)
                    },
                    FromPort: 2049,
                    IpProtocol: 'tcp',
                    ToPort: 2049
                ...
                Image: {
                    "Fn::Join": [
                        ...
                        [
                                ...
                                "Fn::Select": [
                                    4,
                                    {
                                        "Fn::Split": [
                                            ":",
                                            {
                                                "Fn::ImportValue": Match.stringLikeRegexp(`${common.stackName}:ExportsOutputFnGetAtt${props.ecrRepository.node.id.replace('-', '')}`),
                                            },
                                        ],
                                    },
                                ],
                            ...
                            "/",
                            {
                                "Fn::ImportValue": Match.stringLikeRegexp(`${common.stackName}:ExportsOutputRef${props.ecrRepository.node.id.replace('-', '')}`),
                            },
                            ":latest",
                        ],
                    ],
...
describe('EmbeddedLinuxCodeBuildProjectStack Custom', () => {
    ...
    test('adds custom policy statements when provided', () => {
        template.hasResourceProperties('AWS::IAM::Policy', {
            PolicyName: Match.stringLikeRegexp(`EmbeddedLinuxCodeBuildProjectRoleDefaultPolicy*`),
            PolicyDocument: {
                Statement: Match.arrayWith([Match.objectLike(
                    {
                        Action: 's3:GetObject',
